...
public class GraphicsConsole extends JFrame implements MouseListener, MouseMotionListener, MouseWheelListener {
	...
	public GraphicsConsole(boolean setUndecorated, int width, int height) {
		super.setUndecorated(setUndecorated); 
		makeGUI(width, height, DEFAULT_FONTSIZE, DEFAULT_NAME);	
	}
	 ...
	 *            The height of the rectangle that the star is inscribed in.
	 */
	public void fillStar (int x, int y, int width, int height, double angle) {
		int[] xPoints, yPoints;
		float rx, ry, xc, yc, yRad;
		...
		for(int i = 0; i < 11; i++) {
			...
			xPoints[i] = (int) (newx + xc);
			yPoints[i] = (int) (newy + yRad);
		}
		canvas.fillPolygon (xPoints, yPoints, 11);
	}
	...
	public void fillMapleLeaf (int x, int y, int width, int height, double angle) {
		int[] xPoints, yPoints;
		float rx, ry, xc, yc, yRad;
